...
using System.IO;
    ...
    class ColisaoEsquerda : Base
    {
        Mapa m;
        Personagem p;

        public ColisaoEsquerda(Game jogo, Mapa m, Personagem p) : base(jogo)
        {
        this.m = m;
        this.p = p;
        }
        ...
        for (int i = 0; i < m.map[0].getMap().GetLength(0); i++)
        {
            for (int j = (int)(Camera.posizao); j < Camera.fracao_mapa; j++)
            {
                    ...
                    ((m.y - 1 + (i * m.tiles[m.map[0].getMap()[i, j]].size.Y)) + (m.tiles[m.map[0].getMap()[i, j]].size.Y)))))
                {
                    ...
                    {
                        if ((Camera.direcao.X == -1) && ((p.sprite_animado.pos.X) <= (m.x + (j * m.tiles[m.map[0].getMap()[i, j]].size.X) + m.tiles[m.map[0].getMap()[i, j]].size.X + 2)) && ((p.sprite_animado.pos.X) >= (m.x + (j * m.tiles[m.map[0].getMap()[i, j]].size.X) + m.tiles[m.map[0].getMap()[i, j]].size.X - 2)))
                        {
                            Camera.Cam.X = 0;
